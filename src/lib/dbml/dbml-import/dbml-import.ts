...
export const importDBMLToDiagram = async (
    ...
    try {
        ...
        const tables: DBTable[] = extractedData.tables.map((table, index) => {
            ...
            // Extract table note/comment - first check tableNotes parameter, then fall back to table's own note
            let tableComment: string | undefined = tableNotes?.get(tableName);
            if (!tableComment && table.note) {
